...
async function sendTelegram({ signal, price, trade }) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  const chatId = process.env.TELEGRAM_CHAT_ID;
  if (!token || !chatId || !trade) return null;

  const text = [
    `[Y & K] BTC AI Bot: ${signal.label}`,
    `Action: ${trade.side}`,
...
export default async function handler(request, response) {
  ...
  try {
    const previousState = await getBotState();
    const result = await evaluateBot(previousState);
    const state = await upsertBotState(result.state);
    const trade = await insertTrade(result.trade);
    const telegram = await sendTelegram({ signal: result.signal, price: result.price, trade });
